...
test('LightCurve renders with required props', () => {
  // Arrange
  const { getByTestId, queryByTestId } = render(<LightCurve data={testAlerts} />);
...
test('LightCurve points are hidden if pointsAreVisible prop is false', () => {
  // Arrange
  const { getByTestId, queryByTestId } = render(<LightCurve data={testAlerts} pointsAreVisible={false} />);
  const lightCurve = getByTestId(testId);
...
test('LightCurve Point gets circled when mouseOver', async () => {
  // Arrange
  const { getByTestId, queryByTestId } = render(<LightCurve data={testAlerts} dataSelectionCallback={testHandler} />);
  const lightCurve = getByTestId(testId);
  const testPoint = lightCurve.querySelector(pointSelector);
  // Act
...
test('HubblePlot Point grows when clicked and selectionCallback is called', async () => {
  // Arrange
  const { getByTestId, queryByTestId } = render(<LightCurve data={testAlerts} dataSelectionCallback={testHandler} {...testProps} />);
  const lightCurve = getByTestId(testId);
  const testPoint = lightCurve.querySelector(pointSelector);
  // Act
